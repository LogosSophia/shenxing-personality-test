...
# v0.8.4 起不再用“双高题”测高位；保留空字典以兼容旧代码/旧后台字段。
HIGH_PAIR_NAMES = {}
...
def _guard(qid, prompt):
    return _pair(qid, "G", prompt, "符合", {"guard": 1}, "不符合", {"guard": 0}, "护卫维持")


def _stress(qid, prompt, options):
    return _single(qid, "X", prompt, options, "压力协同")


def _domain(qid, prompt, options):
    return _single(qid, "D", prompt, options, "四域底色")
...
QUESTIONS = [
    ...
    _domain("D01", "当事情变复杂时，我最需要先抓住：", [
        {"key": "A", "text": "判断标准、结构关系和哪里说得通。", "scores": {"T": 1}},
        {"key": "B", "text": "主线走向、可能变化和它最终会通向哪里。", "scores": {"N": 1}},
        {"key": "C", "text": "现实条件、身体状态、节奏和可执行落点。", "scores": {"S": 1}},
        {"key": "D", "text": "真实意愿、价值感、人的感受和关系影响。", "scores": {"F": 1}},
    ]),
    _domain("D02", "当我要把一件事推进下去时，我更自然依靠：", [
        {"key": "A", "text": "清楚的逻辑、责任边界和可解释结构。", "scores": {"T": 1}},
        {"key": "B", "text": "对方向、机会、转机和潜在可能的捕捉。", "scores": {"N": 1}},
        {"key": "C", "text": "稳定节奏、具体步骤、现实反馈和可复现方法。", "scores": {"S": 1}},
        {"key": "D", "text": "内在认同、关系连接、感受判断和价值选择。", "scores": {"F": 1}},
    ]),
    _domain("D03", "当一个问题反复出错时，我最先怀疑：", [
        {"key": "A", "text": "判断框架、规则或责任结构出了问题。", "scores": {"T": 1}},
        {"key": "B", "text": "主线、方向或可能路径看错了。", "scores": {"N": 1}},
        {"key": "C", "text": "流程、习惯、现场条件或执行细节不稳。", "scores": {"S": 1}},
        {"key": "D", "text": "真实动机、感受连接或价值关系出了偏差。", "scores": {"F": 1}},
    ]),
    _domain("D04", "最能让我觉得一件事“可以继续”的是：", [
        {"key": "A", "text": "它说得通，有清楚结构和判断依据。", "scores": {"T": 1}},
        {"key": "B", "text": "它有主线，有未来空间和可能展开。", "scores": {"N": 1}},
        {"key": "C", "text": "它落得住，有稳定节奏和现实支撑。", "scores": {"S": 1}},
        {"key": "D", "text": "它被我认可，也能安顿人和关系。", "scores": {"F": 1}},
    ]),

    _guard("G01", "当我想清楚一件事后，我会自然形成某种保存方式，避免以后又重新混乱。"),
    _guard("G02", "我会保留一套自己熟悉的处理方式，哪怕它不完美，也能防止事情失控。"),
    _guard("G03", "当外部变化太快时，我会启动某种稳定机制，让自己不要被带偏。"),
    _guard("G04", "我不只是解决眼前问题，也会想办法让同类问题以后更不容易再次击穿我。"),
    _guard("G05", "有些习惯、记录、边界或流程，对我来说像是王国的保险装置。"),
    _guard("G06", "压力越大，我越需要某种固定方式来保存判断、稳定状态或防止过程漂移。"),
    _guard("G07", "我有时会用一套“不一定喜欢但很有效”的方式，把冲突隔离在核心之外。"),
    _guard("G08", "别人看到的某些稳定习惯或处理方式，其实是在保护我更核心的部分。"),

    _stress("X01", "当某个弱点被戳中、压力开始逼近时，我最自然的第一层处理是：", [
        {"key": "A", "text": "先把压力抽成结构和关键判断，找出它在整条主线里到底算什么。", "scores": {"TN": 1}},
        {"key": "B", "text": "先把目标、责任、步骤立起来，直接接管现场，让事情重新可控。", "scores": {"TS": 1}},
        {"key": "C", "text": "先守住真实感、意义和人的理解，不让那个弱点直接定义我是谁。", "scores": {"NF": 1}},
        {"key": "D", "text": "先回到身体、生活节奏和稳定关系里，让状态不要继续塌下去。", "scores": {"SF": 1}},
    ]),
    _stress("X02", "当别人用结果、效率、评价或群体压力压过来时，我更容易：", [
        {"key": "A", "text": "抽离现场，重新界定问题、标准和主线，不被对方的节奏带走。", "scores": {"TN": 1}},
        {"key": "B", "text": "提高执行强度，用更明确的安排、资源调度和现场表现把局面压回去。", "scores": {"TS": 1}},
        {"key": "C", "text": "回到“这是不是我认可的”“人有没有被理解”这一层，拒绝被外部评价吞掉。", "scores": {"NF": 1}},
        {"key": "D", "text": "先降低刺激，稳住关系气氛、身体感受和具体生活，不让场面继续失控。", "scores": {"SF": 1}},
    ]),
    _stress("X03", "当一个问题已经失控，必须马上处理时，我更像是：", [
        {"key": "A", "text": "先找出背后的结构、关键变量和整体走向，再决定怎么动。", "scores": {"TN": 1}},
        {"key": "B", "text": "先接管现场，把目标、责任、流程和结果重新立起来。", "scores": {"TS": 1}},
        {"key": "C", "text": "先确认意义、真实意愿、可能出口和人的处境，避免越处理越偏。", "scores": {"NF": 1}},
        {"key": "D", "text": "先处理具体体验、习惯秩序、关系连接和现实安顿，让人能继续撑住。", "scores": {"SF": 1}},
    ]),
    _stress("X04", "当外部世界变得混乱、吵闹或压迫时，我最自然的自保方式是：", [
        {"key": "A", "text": "退到解释框架和主线里，把混乱重新变成可理解的问题。", "scores": {"TN": 1}},
        {"key": "B", "text": "进入行动和现场里，把混乱重新变成可分配、可执行、可控制的任务。", "scores": {"TS": 1}},
        {"key": "C", "text": "退到真实感、意义和可能里，不让现实结果或噪音完全定义我。", "scores": {"NF": 1}},
        {"key": "D", "text": "退到稳定生活、身体节奏和亲近关系里，先把自己和人安顿住。", "scores": {"SF": 1}},
    ]),
]
