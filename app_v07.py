...
def _kingdom_role_rows(result):
    ...
    return [
        {"王国位次": "君主", "结构面向": "核心原则", "功能": m["monarch"], "关键词": PRINCIPLES[m["monarch"]], "分数": round(scores.get(m["monarch"], d.get("monarch_raw", 0)), 2), "补充": "原则排名用于确认君主是否在前列"},
        {"王国位次": "宰相", "结构面向": "施政原则", "功能": m["chancellor"], "关键词": PRINCIPLES[m["chancellor"]], "分数": round(d["chancellor"], 2), "补充": "由王国模板推出，不再直接询问"},
...
with st.expander("题库导出", expanded=False):
    rows = []
    for q in QUESTIONS:
        rows.append({"题号": q["qid"], "部分": q["module"], "题型": q["question_type"], "题目": q["front_text"], "选项": " / ".join(_option_label(option) for option in q.get("options", []))})
...
if st.button("生成测评结果", type="primary"):
    ...
    for f in result.get("principle_order", []):
        ...
        principle_rows.append({"排名": int(p["Rank"]), "原则": f, "关键词": PRINCIPLES[f], "最终分": round(p["PrincipleScore"], 2), "四域": DOMAIN_NAMES[p["Domain"]], "四域分": round(p["DomainScore"], 2), "域内分": round(p["DirectionScore"], 2), "混战分": round(p["MixedScore"], 2)})
    ...
    with st.expander("查看详细数据（可选）"):
        ...
        for candidate_type, display_score in result["ordered_types"]:
            ...
            candidate_rows.append({"类型": candidate_type, "王国模板分": round(display_score, 3), "君主": TYPE_MAP[candidate_type]["monarch"], "君主分": round(cd["monarch_raw"], 3), "宰相": TYPE_MAP[candidate_type]["chancellor"], "帝师": TYPE_MAP[candidate_type]["emperor"], "轴显形加分": round(cd.get("role_axis_bonus", 0), 3), "君主排名惩罚": round(cd.get("monarch_rank_penalty", 0), 3)})
